...
ScannBuilder::ScannBuilder(ConstDataSetWrapper<float, 2> db, size_t num_neighbors, std::string distance_measure)
    : db_(std::move(db)), num_neighbors_(num_neighbors), distance_measure_(distance_measure)
{
}
...
ScannBuilder & ScannBuilder::Tree(
    size_t num_leaves,
...
ScannBuilder & ScannBuilder::ScoreAh(
    ...
    size_t training_iterations)
{
    score_ah_args_ = detail::ScoreAHArgs{
        dimension_per_block,
...
ScannBuilder & ScannBuilder::Reorder(size_t reordering_num_neighbors, bool quantize)
{
    reorder_args_ = detail::ReoederArgs{reordering_num_neighbors, quantize};
    return *this;
}

scann::ScannSearcher ScannBuilder::Build(DB::ContextPtr /*context*/)
{
    // if (context == nullptr) {
    ...
    return {
        "/home/ubuntu/disk/NikeVas/ClickHouse/contrib/google-research/scann/"
...
std::string ScannBuilder::GenerateTreeConfig(detail::TreeArgs & args)
{
    ...
    std::sprintf(
      ...
      partitioning {\n\
        ...
        max_spill_centers: %zu\n\
        }\n\
        expected_sample_size: %zu\n\
...
std::string ScannBuilder::GenerateScoreAHTreeConfig(detail::ScoreAHArgs & args)
{
    ...
    else
    {
        ...
        variable_blocks {\n\
          num_blocks: %luu\n\
          num_dims_per_block: %zuu\n\
        }\n\
        variable_blocks {\n\
          num_blocks: %d\n\
          num_dims_per_block: %lu\n\
        }\n\
          ",
            args.n_dims / args.dimension_per_block,
            args.dimension_per_block,
    ...
    size_t num_blocks = std::ceil(static_cast<double>(args.n_dims) / args.dimension_per_block);
    bool global_topn = (args.hash_type == "lut16") && (num_blocks <= 256) && args.residual_quantization;
    ...
    std::sprintf(
      ...
      hash {\n\
        ...
        BoolToString(global_topn),
        clusters_per_block,
        args.n_dims,
        proj_config,
        args.anisotropic_quantization_threshold,
        args.training_sample_size,
...
std::string ScannBuilder::GenerateScoreBruteForceConfig(detail::ScoreBruteForceArgs & args)
{
    char complete_str[256];
    std::sprintf(complete_str, "\n\
      brute_force {\n\
        fixed_point {\n\
          enabled: %s\n\
        }\n\
...
std::string ScannBuilder::GenerateReorderConfig(detail::ReoederArgs & args)
{
    ...
    std::sprintf(
        ...
        "\n\
      exact_reordering {\n\
        approx_num_neighbors: %zu\n\
        fixed_point {\n\
          enabled: %s\n\
        }\n\
      }\n\
    ",
        args.reordering_num_neighbors,
        BoolToString(args.quantize));
    return complete_str;
}
...
std::string ScannBuilder::CreateConfig()
{
    ...
    {
        score_ah_args_.value().residual_quantization = tree_args_.has_value() && distance_measure_ == "dot_product";
